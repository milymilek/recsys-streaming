...
def _preprocess_data(session, df_ratings, df_metadata, output_path):
    ...
    dump_feature_maps(
        user_id_map=USER_ID_MAP, parent_id_map=PARENT_ASIN_MAP, store_id_map=STORE_MAP
    )
    ...
    map_parent_asin_udf = session.udf.register(
        "map_parent_asin", map_parent_asin, IntegerType()
    )
    map_store_udf = session.udf.register("map_store", map_store, IntegerType())

    # Apply the UDFs to the DataFrame
    df_mapped = (
        df_filtered.withColumn("user_id", map_user_id_udf(col("user_id")))
    ...
    df_mapped.coalesce(1).write.format("csv").mode("overwrite").option(
        "header", "true"
    ).save(output_path)
...
def process_data(df_rating_stream=None):
    ...
    _preprocess_data(
        session, df_ratings, df_metadata, output_path=DATASET_FILE.as_posix()
    )
