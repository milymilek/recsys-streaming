...
def load_feature_maps(
    path: pathlib.Path = FEATURE_MAPS_FILE.with_suffix(".pkl"),
) -> dict[str, int]:
...
def read_item_feature_store(db, feature_maps, collection="metadata"):
    ...
    item_feature_store["parent_asin"] = item_feature_store["parent_asin"].map(
        feature_maps["parent_id_map"]
    )
    item_feature_store["store_id"] = item_feature_store["store"].map(
        feature_maps["store_id_map"]
    )
    item_feature_store = (
        item_feature_store.drop(columns="store")
        .dropna()
        .astype(int)
        .sort_values(by="parent_asin")
