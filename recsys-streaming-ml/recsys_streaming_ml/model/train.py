...
def train(epochs, batch_size, validation_frac, cuda, seed):
    ...
    X_train, X_valid, y_train, y_valid = cast_df_to_tensor(
    ...
    feature_sizes = (
        len(feature_maps["user_id_map"]),
        len(feature_maps["parent_id_map"]),
        len(feature_maps["store_id_map"]),
    )

    print("> Feature maps loaded")

    model = DeepFM(emb_dim=8, hidden_dim=[32, 24, 10], feature_sizes=feature_sizes).to(
        device
    )
    ...
    for epoch in tqdm(range(epochs)):
        ...
        test_loss, test_roc_auc = test(
            model=model, criterion=criterion, val_loader=val_loader, device=device
        )
...
def run():
    ...
    args = parse_args()
    train(args.epochs, args.batch_size, args.validation_frac, args.cuda, args.seed)
