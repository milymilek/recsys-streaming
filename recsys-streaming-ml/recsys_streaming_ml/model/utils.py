...
def neg_sampling(X, y, k=1, item_cols_idx=[1, 2]):
    ...
    return torch.cat([X, X_neg_sampl], axis=0), torch.cat(
        [y, torch.zeros(X_neg_sampl.shape[0], 1)], axis=0
    )
...
def load_model_from_db(db: Database, device: str = "cpu"):
    ...
    print(
        f'Loading model: {model_document["model"]}:v{model_version}-{model_document["timestamp"]}'
    )
